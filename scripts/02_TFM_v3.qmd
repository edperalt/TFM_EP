...
well_top_base_ss %>% 
  ...
  geom_label(aes(y=3400, x = 12, label = "Sección de pozo con núcleo"), color = "purple")+
...
En la @fig-xsection observamos que la media de profundidad de la formación esta cercana a los 3000 metros bajo el nivel del mar, además vemos que los núcleos estan aproximadamente a la misma profundidad y representan una proporcion baja de las muestras de la formacion Brent
...
En la @tbl-algo_imput presentamos los distintos métodos de imputación seleccionados para cada medida:
...
#| warning: false
#| message: false
#| tbl-cap: "Algoritmos utilizados para imputar las distintas medidas"
...
-   Impedancia acústica, @acoustic es el producto de la velocidad sísmica y la densidad y normalemnte se denota con la letra Z, en este caso la llamaremos **ed_Z**. La velocidad sísmica es el inverso de la curva del sónico **DTC**.

Adicionalmente si observamos la @fig-xsection es claro que el rango de profundidad en el que encontramos la formación es es bastante amplio, esto normalmente tiene efectos en algunas de las mediciones que realizamos, por lo que hemos buscado las correlaciones existentes entre la profundidad bajo el nivel del mar y las distintas medidas. En los casos que encontramos cierto grado de correlación lineal hemos generado nuevas variables que contienen el residual obtenido al eliminar la tendencia identificada. Estas variables tienen el prefijo "**re\_**".
...
#| warning: false
#| message: false
#| tbl-cap: "secci'on de pozo con nucleo en metros."


cored_brent %>% group_by(well) %>%
  summarise(seccion_con_nucleo = round(max(DEPT) - min(DEPT), 1)) %>%
  arrange(desc(seccion_con_nucleo)) %>%
  rename(pozo = well) %>% 
  adorn_totals() %>% 
  flextable() %>%
  autofit() %>%
...
seccion_brent <- 
  logs_brent %>% group_by(well) %>%
  select(well, brent, DEPT) %>% filter(brent == T) %>%
  summarise(espesor_brent = max(DEPT) - min(DEPT)) %>%
  ungroup() %>%
  summarise(espesor_brent = sum(espesor_brent))


```

Tenemos un total de 840 metros de datos con núcleos lo cual representa un `r round(840 / seccion_brent[[1]] * 100,0)` % del los `r round(seccion_brent[[1]],0)` metros perforados de la formación Brent. Esto es un valor relativamente alto a los promedios de la industria que normalmente rondan el 5% .

En la @fig-tipoderoca podemos ver que nuestro set de datos es bastante desbalanceado, comenzaremos analizando la distribución de las propiedades petrofísicas que afectan los volúmenes y la producción de petróleo. Porosidad y Permeabilidad.
...
xdata <- read_rds("../data/ing_var_v1.rds") %>% select(-c(FORCE_2020_LITHOFACIES_LITHOLOGY, p5, p95))
...
#| warning: false
#| echo: false
#| fig-cap: "Porosidad y Permeabilidad en los núcleos por tipo de roca"
#| fig-subcap:
#|   - "Curva de dendsidad de la porosidad por tipo de roca"
#|   - "Curva de densidad de la permeabilidad por tipo de roca" 
#| layout-ncol: 2
#| column: page-right
xcore %>% 
  ggplot(aes(ed_denporo, color = lith_name))+
  geom_density()+
  theme_minimal()+
...
xcore %>% 
  ggplot(aes(ed_perm, color = lith_name))+
  geom_density()+
  theme_minimal()+
  labs(color = "tipo de roca")+
  scale_x_log10()+
...
De los gráficos confirmamos que el des-balance de muestras nos puede generar problemas a futuro, y problemas relacionados a la predicción de la producción , ya que tenemos mucho solapamiento de las distribuciones de todos los tipos de roca, siendo el Carbón el que mayor diferenciación presenta en estos espacios.

A Continuación vamos a revisar la relación de todas las variables continuas con los tipos de roca, en general las curvas de resistividad y de permeabilidad tienen comportamientos logarítmicos por lo que vamos a transformar para la visualización las variables mencionadas al espacio logarítmico, las variables relacionadas con las coordenadas de pozo las vamos a eliminar de la comparación ya que no tiene sentido considerarlas en estos gráficos, así como el tamaño de broca (**bit_size**).

```{r}
#| label: fig-boxplot_train
#| message: false
#| warning: false
#| fig-width: 10
#| fig-height: 10
...
xcore %>% 
  select(lith_name, where(is.numeric), -c(DEPT, x_loc,y_loc, z_loc, bit_size) ) %>%
  pivot_longer(-lith_name) %>% 
  ggplot(aes(value, color = lith_name))+
  geom_boxplot() +
...
En general se observan cosas interesantes, de los mas importantes:

-   La curva del neutrón (NPHI) parece poder separar bien los tipos de roca con poco solapamiento entre los distintos valores.

-   la combinación de la medida de los rayos gamma normalizado (ed_gr_norm) y el sin normalizar (GR) se complementan para permitir buscar un espacio que permita la identificación de los distintos tipos de roca.

visualizamos antes de continuar un gráfico de coeficiente de correlación de todas las variables
...
#| echo: false
#| fig-cap: "Descripción de los tipos de roca en los núcleos"
plot_correlation(xcore %>% select(-c(well, x_loc, y_loc, z_loc, bit_size, DEPT)))
```

Es claro que algunas transformaciones mantienen una alta correlación lineal con la variable inicial, como es el caso de la porosidad y la densidad, también tenemos alta correlación entre las distintas resistividades, será interesante observar cuales de estas variables mantenemos después de un ejercicio de selección de variables tipo *stepwise*.

```{r}
#| label: fig-barplot_formation
#| #| message: false
#| warning: false
#| fig-width: 8
#| fig-height: 10
...
xcore %>% 
  ggplot(aes(lith_name, fill = fm))+
  geom_bar()+
  theme_minimal()+
  theme(axis.title.y = element_blank())
```

Aunque las muestras están lejos de ser exhaustivas vemos que en principio deberiamos esperar Carbon (Coal) solamente en la formación Tarbert, así como es poco probable que encontremos esquistos o **shale** en la formación Tarbert.

```{r}
#| label: fig-barplot_bit_size
#| #| message: false
#| warning: false
#| fig-width: 8
#| fig-height: 10
...
#| 
xcore %>% 
  ggplot(aes(lith_name, fill = as.factor(bit_size)))+
  geom_bar()+
  theme_minimal()+
  theme(axis.title = element_blank(),
        )+
  labs(fill = "tamaño de ebroca") 
```

La medida de el tamaño de broca lo vamos a descartar básicamente porque es una decisión que no se ve afectada por el tipo de roca que tenemos,es una decisión económica que depende del objetivo final del pozo.

```{r}
#| label: fig-barplot_disc
#| #| message: false
#| warning: false
#| fig-width: 8
#| fig-height: 10
...
plot_bar(xcore %>% mutate(bit_size =  as.factor(bit_size)), by= "lith_name",ggtheme = theme_minimal())
...
#| warning: false
#| echo: false
#| fig-cap: "qq-plot de todas las variables continuas"
#| fig-subcap:
#|   - "Curva de dendsidad de la porosidad por tipo de roca"
#|   - "Curva de densidad de la permeabilidad por tipo de roca"
#|   -  "hola" 
...
DataExplorer::plot_qq(xcore[,c("re_nphi", "re_phie", "re_ed_denporo", "re_rhob_clean", "re_dtc", "re_gr", "re_sp", "log_perm", "ed_perm", "re_sp","log_ind_perm", "ed_ind_perm", "ed_phie", "ed_denporo", "ed_vsh", "ed_gr_norm", "rhob_clean", "NPHI", "DTC", "log_RSHA", "RSHA", "log_RDEP","RDEP", "CALI", "GR", "bit_size", "pef_clean", "DRHO", "SP", "log_RMED", "RMED")],ggtheme = theme_minimal())



```

Es claro que la transformación que aplicamos sobre las medidas de resistividad y permeabilidad han funcionado para obtener distribuciones de los datos que se aproximan a una distribución normal.
...
HEmos realizado. el proceso de Stepwise para explorar diversos conjuntos de variables que nos permitan evaluar grupos de algoritmos.
