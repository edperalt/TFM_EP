...
no_model_data <- for_input %>% select(well, fm, brent, lith_name, FORCE_2020_LITHOFACIES_LITHOLOGY, bad_data, x_loc, y_loc, z_loc)

imput_rec <- recipe(DEPT ~ . , data = for_input %>% 
                      select(-c(well, fm, brent, lith_name, FORCE_2020_LITHOFACIES_LITHOLOGY, bad_data, x_loc, y_loc,z_loc)) )
...
imputed_data <-
  imputed_bag %>% 
  select(well, DEPT, x_loc, y_loc, z_loc, RMED, SP, DRHO, pef_clean, FORCE_2020_LITHOFACIES_LITHOLOGY,bit_size,lith_name, fm) %>%
  cbind(imputed_mean %>%
          select(GR)) %>%
  cbind(imputed_knn %>%
